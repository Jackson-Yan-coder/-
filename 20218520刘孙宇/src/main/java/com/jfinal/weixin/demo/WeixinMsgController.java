...
import com.jfinal.weixin.util.DianYingUtil;
import com.jfinal.weixin.util.MusicUtil;
...
public class WeixinMsgController extends MsgControllerAdapter {
    ...
    private static HashMap<String, Users> userMap = new HashMap<>();
    ...
    protected void processInTextMsg(InTextMsg inTextMsg) {
        ...
        if(userMsg.startsWith("#"))
        {
            ...
            {
                StringBuffer sb = new StringBuffer("1.搜索音乐格式为：#音乐@***\n");
            ...
            }else if(cmd.equals("电影"))
            {
                ...
                String value = null;
                try {
                    value = dyUtil.searchMusic(search,users.getDy_page());
                    value+="下一页请输入：%%3";
                    outMsg.setContent(value);
                    render(outMsg);
                } catch (UnsupportedEncodingException e) {
...
//                        value="上一页请输入：!!1\n";
//                    }
                    value += dyUtil.searchMusic(user.getDy_search(),page);
                    if(value==null||value.equals("")){
                        outMsg.setContent("已经是最后一页了，请返回第一页查询");
                        render(outMsg);
                    }else {
                        System.out.println(value);
                        value+="下一页请输入：%%3";
            ...
            News news=new News();
            news.setTitle("点击观看电影");
            news.setUrl("http://lsy20030717.gnway.cc/dy?dyId="+search);
